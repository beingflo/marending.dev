...
for example, a margin of a list item only if it precedes an image element, this may
...
loaded or not. I noticed that sometimes particular images were loaded and sometimes not,
which defeats the purpose when trying to look for pixel differences between snapshots.
...
run the tests whenever I feel like I've made changes that could affect some other part of 
the site. There is no point in burning energy by running them on every commit or constantly
failing my deployment just to confirm that changing a typo on a page does in fact cause
visual changes.
