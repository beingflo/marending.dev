...
[blog post on lab notebooks](https://sambleckley.com/writing/lab-notebooks.html). The author argues
...
the time I read the post I knew of no one who kept such notes.

Sure, we have documentation (maybe), but that's not the same thing. Documentation is there to
explain how a system works, how to interact with it, maybe what constraints it operates under. But
documentation typically doesn't explain _how_ and _why_ we arrived at this particular solution. Did
...
remembers _why_ it was built the way it is. But I would often have this nagging feeling that we
considered the proposed solution back then but decided against it for _reasons_. Now the big
...
[notes app](/notes/fieldnotes), I had the feeling I needed something more specialized for the
occasion. An application that would keep entries in a chronological feed, rather than siloed notes.
Enter [jour](https://github.com/beingflo/journal), a simple web app for keeping journal entries with
no hassle. It's local-first with optional S3 sync. A hosted version is available at
[jour.rest.quest](https://jour.rest.quest/). As you can tell by the URL, this application is another
instance of my local-first series kicked off by [rest.quest](/notes/rest-quest). I would recommend
reading that project report first for the technical details, here I will only go into the feature
set and learnings from this project.
...
by looking at words starting with a `#` in the last line. Tags do not get special treatment other
than being displayed in a lighter tone below the content.

![Screenshot of jour](./_view.png)

On the top, there is a search bar that searches through all entries and filters down the feed
...
tagged by a special tag (`nca` in my case). Instead of always having to type out this tag manually,
I later added the things you see on the left. When clicking on `nca`, there is an implicit filter
...
Next, by hitting `n`, a new entry is created at the top with a textbox and a date picker. By default
the datepicker shows the current date and time, but I quickly found that sometimes one wants to
create entries in the past, so I added the ability to set the date. With `cmd+Enter` the entry is
saved.
...
Here you can see that I'm quite consistent in using jour. The week(s)-long gaps are almost always
vacations.

At the time of writing there are 848 entries saved. Many of them quite short, some are longer. There
...
grows to a point where this becomes laggy, I'll reconsider.
...
[rest.quest](/notes/rest-quest) as a todo list and [fieldnotes](/notes/fieldnotes) as the notes app.
...
discussed in a sync meeting to refer to later. Dedicated note or quick entry in jour?

This conundrum has kicked off thoughts I can't fully verbalize yet. In classical _me_ fashion, one
...
doing too many things badly is not exactly a mistake I want to repeat. We'll see where this goes.
