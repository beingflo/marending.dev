...
is an implicit filter for this term that doesn't show up in the search bar, and every new
...
At the time of writing there are 848 entries saved. Many of them quite short, some are longer.
There is no progressive loading of entries, everything is shown on one page. 
Turns out you can go quite far with such a simple UI when you build it to your
specifications. If the sheer number of entries grows to a point where this becomes laggy,
I'll reconsider.
...
an indecisiveness about where to record some things. For instance, after figuring out a shell
...
Or taking notes on what's being discussed in a sync meeting to refer to later. Dedicated
note or quick entry in jour?
