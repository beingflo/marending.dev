...
Next, we need to describe our schedule in an xml file. Here the one for the window title
recording. The script is called `window.sh`. Make sure to use absolute paths everywhere, 
...
installed correctly. I just set the interval low at first and see it works, 
