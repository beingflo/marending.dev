...
Now we see `data` with its `metadata`, `value` and `typed_value` properties and also `value` and
`typed_value` logical columns for both `name` and `age`. For these the shredded types are UTF8 and
INT32 respectively.
...
The double casting to JSON and then Variant is apparently doing the heavy lifting to ensure nested
...
<Hint>
    ...
    performance, and then a sudden jump in performance as it becomes backed by Variant.

    I was reinvigorated to check whether my benchmark queries would now be faster in the preview version
