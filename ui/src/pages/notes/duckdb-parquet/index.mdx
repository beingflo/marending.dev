...
world data from my existing observability setup. Luckily ClickHouse makes it really easy to get my
data out: `SELECT * FROM events.otel_traces INTO OUTFILE './otel_traces.parquet' FORMAT Parquet`.
This dumps all of the data into a single Parquet file with just over 10 milion rows and weighing 800
MB.

The shape of this file is determined by the standard schema the OpenTelemetry collector created when
I started ingesting data. In ClickHouse this looks like this:
...
You can see a clear correspondance between the Parquet and ClickHouse types. Notably, this file is
...
data and access logs to this website. For both data sets I'll run a query that can benefit from
...
For the GPS data, the first query should get distinct days I've visited a particular location. And
the second query will get average speed and altitude hourly for the whole timespan.
...
rather stay string values to the top-level properties. But as the DuckDB JSON type is anyhow just
...
coordinates into a map and is left as an excercise to the reader._
...
from an even older system into the one I use currently. If want I'm trying here comes to fruition,
...
For the Map type I could just leave the data as is as the original Parquet file already has the
...
Finally, let's get to real meat of the story. Here we store the gps location data into a Parquet
...
This one again is not going to be an actual option for my usecase as I need to know the schema up
...
Now let's look towards another dataset I have in my observability system: Access logs to this
...
The way OpenTelemetry ingests this data, the `Events.Attributes` property then takes the following
...
Now, going through the same spiel again, here we extract the relevant data into a Parquet file with
the relevant data cast to JSON.
...
cared to know. `.env` is not much further down the list though.
...
one I actually cared about, Variant, is dissapointing. I might be hitting
...
this data set and it churns through that in no time even with JSON types.

Interesting are also the file sizes of the different parquet files, one can clearly see how the
...
harder. We must be hitting a highly unoptimized code path here to be that much slower to even JSON
handling.
...
I don't want to get to deep into this, as the goal here is primarily to establish the feasibility of
querying my data from a Parquet file. But just to get a feel for the landscape of query engines, I'm
going to translate the 4 queries I used here to ClickHouse equivalents and run them against the live
system. Note, that this is very much an apples to oranges comparison: The numbers thus far have been
achieved on a M1 Macbook Air, the ClickHouse ones on a Hetzner CAX21. And the Parquet files have
first been isolated to their respective datasets, ClickHouse has an asortment of data in its
`otel_traces` table. Not to mention that the Hetzner server is a noisy shared VPS.

<Clickhouse_results />

The closest equivalent of the types we've looked at so far to the current ClickHouse schema would
beny the `Map` flavour. Honestly quite surprised. Goes to show that while you want to get to those
single-digit milisecond latencies, in real world usage these fractions of a second have been just
fine. I would suspect we could go considerably faster by using ClickHouses own JSON type, which
