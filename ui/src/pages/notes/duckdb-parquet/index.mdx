...
file. But somehow I see worse performance here than with JSON, which doesn't make much sense. I
might be hitting [this issue](https://github.com/duckdb/duckdb/issues/22024). Although at the time
of writing, I get similar performance on a nightly build of DuckDB even though it's supposedly fixed
on main.
...
one I actually cared about, Variant, is dissapointing. I think this must truly be a limitation in
DuckDB that will be addressed at some point. Looking at the query plan with `EXPLAIN ANALYZE`, it's
clear that a lot of variant extractions happen, even though in my understanding it shouldn't have to
do that. In a follow-up note I'll have to see how different query engines handle this.
