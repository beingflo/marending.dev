...
I've had observability on my mind again lately. I already considered how to
[store unstructured data](/notes/unstructured-data/) a while ago. After a brief stint of building my
own system based on DuckDB, I've been running an off-the-shelf combination of ClickHouse and Grafana
to collect and visualize tracing data. It has served me well, but a planned move of my side projects
to my own hardware has me re-evaluating my setup. And ClickHouse is by far the heaviest service I
operate on my server. It randomly fills up my disk with logging tables, too. Look, ClickHouse is an
engineering marvel, but it's not _my_ marvel. It's someone else's marvel. And I have a need to
create my own. Thing, not marvel, I mean. So here we are.

Now, I stumbled across the Parquet file format and in particular its new data type that looks like
it was custom-made to solve all my problems. Maybe not all of them. In this note I want to explore
how DuckDB can write and query these files. If this works out, I may be tempted to build a new
observability system based around
[wide events](https://isburmistrov.substack.com/p/all-you-need-is-wide-events-not-metrics) on these
technologies.
