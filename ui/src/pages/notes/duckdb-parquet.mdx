...
To get an overview of the performance of the Variant type I'm going to focus on two types of data I
have stored here, GPS location data and access logs to this website. For both data sets I will come
up with two queries, one that touches only a fraction of the data and one that has to do sweeping
aggregations across most of the data set. Then I'm going to wrangle the payload into the following
types to see how the different approaches stack up:
...
two scenarios: gps location data and marending.dev access logs. for each: one query that touches
little data and one that touches most

string parsing, JSON type, map type, variant, columns

indices?

small files and mixed files

comparison against clickhouse
